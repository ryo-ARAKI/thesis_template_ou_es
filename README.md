...
日本語文献の定義は， `.bib` ファイルで `language={japanese}` というフィールドを記述することで可能です．
`bib_textbooks.bib` にいくつか日本語の文献を定義しているので参考にしてください．

しかし，このテンプレートをコンパイルすると日本語文献に `japanese` という文字列が出力されてしまいます．
テンプレートファイルを修正してこれを出力しないようにする方法は見つけられていません．
代替策として，参考文献の表示スタイルが定義されている `.bbx` ファイルを修正することが挙げられます．
現在のテンプレートではAIPとAPSのスタイルである `phys.bbx` を利用しているため，このファイルの `\printlist{language}` マクロをコメントアウトすれば望んだ出力が得られます．
